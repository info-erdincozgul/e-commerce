...
import { removeFromCart, setPayment } from "../store/actions/ShoppinCartAction";

export default function PaymentType({ addressId }) {
  ...
  const selectedCard = cardList.find((card) => card.id === selectedCardId);
    ...
    sendRequest({
      ...
      callbackSuccess: (res) => {
        ...
        if (initialLoad && res.data.length > 0) {
          ...
          selectedCardId &&
          !res.data.find((card) => card.id === selectedCardId)
        ) {
  ...
  const submitPayment = () => {
    if (!selectedCard || cartList.length === 0) {
      return;
    }

    const productsPayload = cartList
      .filter((item) => item.checked === true)
      .map((item) => ({
        product_id: item.product.id,
        count: item.count,
        detail: item.detail || "Varsayılan Detay",
      }));
    ...
    sendRequest({
      ...
      callbackError: (error) => {
        ...
        setApiError(error.message || "Order submission failed.");
      },
    });
  };
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="flex-1">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-semibold text-lg text-ebonyClay">
            ...
            {showForm ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <span className="font-semibold text-gray-600 group-hover:text-ebonyClay">
                    Add New Card
                  </span>
                </div>
                {cardList.length > 0
          ...
          <div className="flex flex-col gap-y-6 items-start w-full mx-auto lg:w-80">
            <button
              type="button"
              className={`text-white text-center py-2 rounded py-4 w-full transition 
        ${
          selectedCardId
            ? "bg-ebonyClay hover:bg-gray-700 cursor-pointer"
            ...
            <div className="flex flex-col gap-y-6 text-sm ebonyGray w-full bg-white p-4 rounded-lg shadow-sm border border-mercury">
              <span className="text-xl font-semibold text-ebonyClay">
                Order Summary
              </span>
